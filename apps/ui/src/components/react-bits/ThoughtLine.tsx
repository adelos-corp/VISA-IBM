...
const ThoughtLine: React.FC<ThoughtLineProps> = ({
  ...
  const head = (
    <>
      ...
      {showTimer ? <span ref={timerRef} className="thought-line__timer" data-done={isWorking ? undefined : ''} aria-hidden="true">0.0s</span> : null}
      {collapsible ? <span className="thought-line__chevron" data-on={hasTrace ? '' : undefined} aria-hidden="true"><HugeiconsIcon icon={ArrowDown01Icon} size="1em" strokeWidth={2.2} /></span> : null}
  ...
  return (
    ...
    >
      {collapsible ? (
        <button type="button" className="thought-line__head" data-toggle={toggle ? '' : undefined} aria-expanded={toggle ? open : undefined} tabIndex={toggle ? 0 : -1} onClick={() => { if (toggle) setOpen(v => !v); }}>
          {head}
        </button>
